<template>
  ...
  <div v-if="isTeacher">
    <div class="container mx-16 p-1.5 w-auto border border-gray-200 shadow rounded-lg">
      ...
      <div>
        <p class="text-gray-700 text-xl font-bold text-center">식품 알레르기 조사서</p>
      </div>
      <p class="text-wrap text-center whitespace-pre-line">원아들의 식품 알레르기에 관한 실태를 파악하고

        발병 등에 대한 예방 조치와 제거 및 대체식품의 필요여부를 확인하고자 하오니

        특정 식품에 대한 알레르기 반응이 있는 식품을 확인하여 체크해주시기 바랍니다.</p>
      <div class="text-center whitespace-pre-line border border-gray-200 shadow rounded-lg">
        ※ 알레르기 유발식품
        1. 난류 2. 우유 3. 메밀 4. 땅콩 5. 대두 6. 밀 7. 고등어 8. 게 9. 새우 10. 돼지고기 11. 복숭아
        12. 토마토 13. 아황산류 14. 호두 15. 닭고기 16. 쇠고기 17. 오징어 18. 조개 19. 잣
      </div>
      <div class="flex items-center">
        <template v-for="allergy in allergyList" :key="allergy.no">
          <input :id="'checked-checkbox-' + allergy.no" type="checkbox" :value="allergy.no"
            :checked="isKidAllergic(allergy.no)"
            class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600" />
          <label :for="'checked-checkbox-' + allergy.no"
      ...
      <div>
        <p>반 : {{ kidAllergy.classroomName }}</p>
        <p>이름 : {{ kidAllergy.kidName }}</p>
        <p>서명란 - 추가작업</p>
      </div>
    </div>
  </div>
