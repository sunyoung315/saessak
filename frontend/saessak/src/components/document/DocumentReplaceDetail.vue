		...
		<div v-if="isTeacher">
			...
			>
				...
				>
					...
					<p class="mb-4">날짜 : {{ replacementPerson[0].replacementDate }}</p>
					<p class="mb-4">시간 : {{ replacementPerson[0].replacementTime }}</p>
					<div class="flex justify-start">
						<p class="mb-4">
							보호자 : {{ replacementPerson[0].replacementName }}
						</p>
						<p class="mb-4 ml-48">
							보호자 관계 : {{ replacementPerson[0].replacementRelationship }}
						</p>
					</div>
					<p class="mb-4">
						비상연락망 : {{ replacementPerson[0].replacementNumber }}
					</p>
				</div>
...
const replacementPerson = ref([
	{
		kidId: 1,
		replacementDate: '2024-01-19',
		replacementTime: '16:00',
		replacementVehicle: '자동차',
		replacementRelationship: '할아버지',
		replacementNumber: '010-1111-1111',
		replacementName: '김할범',
