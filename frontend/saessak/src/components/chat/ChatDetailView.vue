<template>
  ...
  >
    <div
      class="fixed w-1/3 top-0 right-0 flex justify-between px-3 py-3 bg-yellow-50 pb-0 mx-auto"
    >
    ...
    >
      <div v-for="msg in chat" :key="msg.chatId" class="flex flex-col mt-5">
        <!--발신 메시지-->
        <div v-if="msg.senderId.endsWith('t')" class="flex justify-end mb-4">
          <div
...
const msgFlag = userFlag == false ? "t" : "k";
console.log(props.roomInfo);
const roomId = props.roomInfo.roomId; // 채팅방 번호
const connected = ref(false); // 소켓 연결 여부
const sockJs = new SockJS("http://localhost:8081/chat");
const recvList = ref([]);
const msg = ref("");
  ...
  console.log("Send message:" + msg.value);
      if (stompClient && stompClient.connected) {
        const sendMsg = {
          ...
          receiverId : props.roomInfo.receiverId + msgFlag,
          senderId: props.roomInfo.senderId + msgFlag,
          chatContent: msg.value
        };
        this.stompClient.send("/pub", JSON.stringify(sendMsg), {});
    }
}
