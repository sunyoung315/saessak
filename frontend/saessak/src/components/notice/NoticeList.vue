...
<template>
  <div class="view-frame p-4">
      ...
      <table class="items-center bg-transparent border-collapse mt-4 w-full">
        ...
        <tbody v-if="noticeList.list.length > 0">
          ...
          <tr v-for="notice in noticeList.list" :key="notice.noticeId"
            ...
            <td class="border-t-0 align-middle border-l-0 border-r-0 whitespace-nowrap p-4" @click="startFix(notice)">
    ...
    <div v-if="noticeList.list.length > 0" class="flex justify-center text-2xl font-bold">
      <button :disabled="paging.pageNo === 1" :class="{ 'text-gray-200': paging.pageNo === 1 }"
        @click="paging.pageNo = paging.pageNo - 1">
        ←
      </button>

      <div v-for="index in noticeList.length">
        <button :key="index" class="m-2 rounded-lg p-2" :class="{ 'bg-nav-orange': paging.pageNo === index }"
          @click="paging.pageNo = index">
          {{ index }}
        </button>
      </div>
      <button :disabled="paging.pageNo === noticeList.length"
        :class="{ 'text-gray-200': paging.pageNo === noticeList.length }" @click="paging.pageNo = paging.pageNo + 1">
        →
      </button>
    </div>
  </div>
