...
const changePaging = async pageNo => {
...
<template>
	<div class="view-frame">
		<div class="table-box">
			<table class="table">
				...
				<tbody v-if="noticeList.list.length > 0">
					...
					<tr v-for="notice in noticeList.list" :key="notice.noticeId"
						...
						<td class="border-t-0 align-middle border-l-0 border-r-0 whitespace-nowrap p-4"
							@click="moveNoticeDetail(notice.noticeId)">
							<div v-if="notice.fileFlag" class="flex justify-start items-center pl-3">
								<svg xmlns="http://www.w3.org/2000/svg" width="20" height="18" viewBox="0 0 20 18"
									fill="none" class="transform rotate-90 inline-block mr-5">
		...
		<div v-if="noticeList.list.length > 0" class="flex justify-center font-bold">
			<button :disabled="paging.pageNo === 1" @click="changePaging(paging.pageNo - 1)" class="m-2">
			...
			<div v-for="index in noticeList.length" :key="index">
				<button class="rounded p-2 text-base px-3 mx-2"
					:class="{ 'bg-nav-orange bg-opacity-60': paging.pageNo === index }" @click="changePaging(index)">
					{{ index }}
				</button>
			</div>
			<button :disabled="paging.pageNo === noticeList.length" @click="changePaging(paging.pageNo + 1)" class="m-2">
				<svg xmlns="http://www.w3.org/2000/svg" width="7" height="12" viewBox="0 0 7 12" fill="none">
					<path
