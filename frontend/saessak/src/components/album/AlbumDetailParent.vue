	...
	<div class="view-frame p-4">
		...
		<div v-if="props.showToggle">
			<div class="flex items-center" v-if="myKidAlbumDateList.length">
				<div class="w-full">
					<div class="m-4 flex flex-wrap" v-if="isSameDate(myKidAlbumDateList[0].albumDate, date)">
						<p class="w-full text-2xl font-bold m-2 px-4">
							{{ myKidAlbumDateList[0].albumTitle }}
						</p>
						<div v-for="file in myKidAlbumDateList[0].fileResponseDtoList" :key="file.fileId"
			...
			<div class="flex items-center" v-if="albumDateAllList.length">
				<div class="w-full">
					<div v-for="album in albumDateAllList" :key="album.albumId">
						<div class="m-4 flex flex-wrap" v-if="isSameDate(album.albumDate, date) &&
...
const download = async () => {
	...
	if (checked.value.length == 1) {
		const fileKey = checked.value[0].replace(removeUrl, '');
		// console.log(fileKey)
		try {
			// S3 객체 다운로드 명령 생성
			...
			downloadLink.download = `image${0}.` + type;
			downloadLink.click();
			downloadLink.remove();
