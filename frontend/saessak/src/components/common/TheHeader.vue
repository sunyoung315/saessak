...
<template>
        ...
        <div v-if="isTeacher == false" class="h-60 overflow-auto">
          ...
          >
            <div class="w-full ps-3 flex justify-between items-center">
              <div>
                <span class="font-semibold text-gray-900">{{ alarm.alarmType }}</span>
      ...
      >
        ...
        >
          ...
          <div class="py-1 bg-gray-200 rounded-b-lg">
            <a @click="newKids()" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-300"
              >아이 등록</a
            >
            <a @click="logout()" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-300"
              >로그아웃</a
            >
          </div>
        </div>
  ...
  >
    ...
    <div
      v-if="flag == false"
      class="fixed w-1/3 bottom-0 right-0 p-3 border-l-2 border-l-gray-300 bg-yellow-50"
    >
